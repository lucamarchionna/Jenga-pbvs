...
vpHomogeneousMatrix homogeneousTransformation(string link1, string link3) 
{
  ...
  try {
    transformStamped = tfBuffer.lookupTransform(link1, link3, t, ros::Duration(0.5));
...
{ 
  ...
  try {
    vpHomogeneousMatrix eeTcam, baseTee, eeTtarget, base2target, targetTcam, cam_desTtarget, camTtarget, cdTc, offset; //since they belong to different trees
    ...
    cdTtarget.buildFrom(cdto, cdRo);    //(cdto, camTtarget.getRotationMatrix());
    std::cout << "cdTtarget : " << cdTtarget << std::endl;
    ...
    double threshold_rotation=8;
    vpColVector v_ee(3), omega_ee(3), v(6);
    
    /*
    while (error>=threshold_rotation && node_handle.ok()) 
    {
      
      //Update camera position
      camTtarget=homogeneousTransformation("camera_color_optical_frame", "handeye_target"); //update position camera wrt 
    ...
    */
    ...
    while (error>=threshold && node_handle.ok()) 
    {
      ...
      //Update camera position
      camTtarget=homogeneousTransformation("camera_color_optical_frame", "handeye_target");       cdTc=cdTtarget*camTtarget.inverse();
      cdTc=cdTtarget*camTtarget.inverse();
